...
export async function createShortUrl(req, res) {
  ...
  if (shortcode) {
    if (!/^[a-zA-Z0-9]{3,10}$/.test(shortcode)) {
      return res.status(400).json({
        message: "Shortcode must be alphanumeric and 3–10 characters.",
      });
    }
  ...
  console.log(`[CREATE] Short URL created: ${code} → ${url}`);
...
export async function redirectToOriginal(req, res) {
  ...
  console.log(`[REDIRECT] ${code} → ${url.originalUrl}`);
...
export async function getUrlStats(req, res) {
  ...
  console.log(`[STATS] Stats fetched for ${code}`);
